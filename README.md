...
프로젝트의 전체적인 큰 틀과 구성 요소에 대한 요약은 [`PROJECT_OVERVIEW.md`](PROJECT_OVERVIEW.md)에서 확인하실 수 있습니다.

> **참고 — 아래 내용은 현재 구현·운영 중인 버전(5축 MOS) 기준입니다.**
> 오디오/음성 차원(억양 적합성, 민감어·욕설 등)을 확장한 다음 버전 설계는
> [`docs/superpowers/specs/2026-07-15-dubbing-qc-v2-audio-redesign.md`](docs/superpowers/specs/2026-07-15-dubbing-qc-v2-audio-redesign.md)에
> 정리되어 있으며, **아직 구현 전(설계 완료 단계)**입니다.
...
## 🔜 다음 방향 (v2, 설계 완료 · 구현 예정)

오디오/음성 차원을 확장하는 재설계가 완료되어 있습니다(스펙: 위 v2 링크 참고).
**아직 코드에 반영되지 않은 계획**이며, 요지는:

- MOS **5축 → 7축**: 억양 적합성(전 세그먼트 미국식 표준 부합 여부), 민감어·욕설
  (별도 콘텐츠 플래그 섹션)을 추가
- **감정 표현 축 확장**: 연출가 페르소나가 더빙 오디오뿐 아니라 **원본 한국어
  오디오도 함께 듣고** 비교 판단 (텍스트로 추론한 "기대 감정"이 아니라 원본의
  실제 연기가 기준)
- **싱크 정확도 재정의**: "입 모양" 대신 원본↔더빙의 **타임코드 격차**로 계산 —
  영상 입력이 어느 축에서도 필요 없어짐
- 화자 분리·화자 임베딩 기반 캐릭터별 억양 비교는 오탐 위험으로 기각, 세그먼트
  독립 분류 방식으로 대체
