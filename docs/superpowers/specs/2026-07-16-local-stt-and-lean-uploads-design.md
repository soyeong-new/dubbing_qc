...
이 설계는 두 문제를 근본적으로 해결하고, 겸사겸사 발견된 검수 탭의 업로드 중복 문제도
함께 정리한다.
...
- **업로드 전 클라이언트 측 오디오 추출(ffmpeg.wasm)** — 초기 논의에서 검토했으나 제외.
  프론트엔드·백엔드가 동일 머신에서 localhost로 통신하므로 업로드는 실제 네트워크 전송이
  아니라 루프백이라 대역폭 병목이 원래 없고, 반대로 ffmpeg.wasm은 파일 전체를 브라우저
  메모리(WASM 힙 + 가상 파일시스템)에 올려야 해서 실사용 파일 크기(2~3GB급)에서
  검증되지 않은 메모리/성능 리스크만 남는다. "가볍고 빠른 분석"에 실제로 기여하는 부분은
  이미 스코프에 포함된 로컬 STT 전환이므로, 이 항목은 이번 설계에서 제외한다.
...
- **스코프 점검:** 5개 컴포넌트 모두 "Gemini STT 의존성 제거 + 검수 탭 업로드 중복 정리"라는
  단일 주제로 수렴하므로 하나의 구현 계획으로 다루기에 적절한 범위다.
